...
# ── Publication style — harmonised across all figures ───────────────────────
rcParams.update(
    {
        "font.family": "serif",
        "font.size": 9,
...
def _panel_label(ax, text, x=0.04, y=0.93):
    """Place bold panel label inside the axes (consistent with Fig 4 style)."""
    ax.text(x, y, r"\textbf{" + text + "}", transform=ax.transAxes, fontsize=8, va="top")
...
def figure_1():
    fig, axes = plt.subplots(1, 3, figsize=(TEXTWIDTH, 2.6))
    ...
    styles = ["-", "-", "-", "--", "--"]
    for k in range(5):
        ax.plot(r, TB(k, r), color=PAL[k], linewidth=1.0, linestyle=styles[k], label=f"$k={k}$")
    ax.set_xlabel("$r$")
...
def figure_2():
    ...
    if data1 is not None:
        for k, col in [(3, PAL[0]), (5, PAL[2]), (7, PAL[3])]:
            ...
            ax.plot(
                r_fine[::5], f_clip[::5], ".", color=col, markersize=2, alpha=0.85, label=f"$k={k}$"
            )
    ax.set_xlabel("$r$")
    ax.set_ylabel("$z_k(r)$")
    ax.set_ylim(-14, 14)
    ...
    ax.legend(
        ...
        frameon=False,
        fontsize=7,
        title="solid: target, dots: recovered",
        title_fontsize=6,
    )
    ...
    if data1 is not None and "k5_phis" in data1:
        ...
        ax.plot(a_demo, P_vals, "-", color=PAL[0], linewidth=1.3, label="$|P(\\tilde{r})| \\leq 1$")
        ax.axhline(1, color=C["gray"], linewidth=0.4, ls=":")

        ax2 = ax.twinx()
        ax2.plot(a_demo, f_clip, "-", color=PAL[1], linewidth=1.0, label="decoded $P/Q$")
        ...
        ax2.annotate(
            "unbounded", xy=(0.96, 12), fontsize=7, color=PAL[1], ha="right", style="italic"
        )
        ...
        ax.legend(lines1 + lines2, labels1 + labels2, loc="center left", frameon=False, fontsize=7)
    else:
        ax.text(
            0.5,
            0.5,
...
def figure_3():
    ...
    if data is not None:
        ...
        ax.set_ylim(1e-17, 3)
        ax.set_xlim(0.5, K_max + 0.5)
        ax.legend(frameon=False, fontsize=6.5, ncol=2, loc="upper right", columnspacing=0.8)
    else:
        for ax in axes[:2]:
            ax.text(
                0.5,
...
def figure_4():
    ...
    if data is not None:
        ...
        ax.plot(
            ...
            linewidth=0.8,
            alpha=0.5,
            label="ideal $\\sqrt{1-a^2}$",
        )
        for i, d in enumerate(d_show):
            P = data[f"d{d}_P_vals"]
            ax.plot(a_fine, np.abs(P), color=PAL[i], linewidth=1.0, label=f"$d={d}$")
        ax.axhline(1, color=C["gray"], linewidth=0.4, ls=":")
        ...
        _panel_label(ax, "(b)")
        ax.plot(
            r_fine, 1.0 / r_fine, color=C["black"], ls="--", linewidth=0.8, alpha=0.5, label="$1/r$"
        )
        for i, d in enumerate(d_show):
        ...
        for i, d in enumerate(d_show):
            ...
            ax.semilogy(r_fine, err + 1e-16, color=PAL[i], linewidth=1.0, label=f"$d={d}$")
        ax.set_xlabel("$r$")
...
def figure_5():
    ...
    if data is not None:
        ...
        _panel_label(ax, "(b)")
        styles = [("-", 1.5), ("--", 1.5), ("-.", 1.5)]
        for (eps, col), (ls, lw) in zip([(0.1, PAL[0]), (0.01, PAL[1]), (0.001, PAL[2])], styles):
            shots = data[f"shots_eps{eps}"]
            ax.semilogy(r_cont, shots, ls, color=col, linewidth=lw, label=f"$\\epsilon = {eps}$")
        ax.set_xlabel("$r$ (signal magnitude)")
...
def figure_6():
    ...
    ax.plot(r, r_tilde, "-", color=PAL[0], linewidth=1.2, label="$\\tilde{r} = r/\\sqrt{1{+}r^2}$")
    ax.plot(r, T3, "--", color=PAL[1], linewidth=1.0, label="$T_3(\\tilde{r})$")
    ax.plot(r, cot3_clip, "-", color=PAL[2], linewidth=1.2, label="$\\cot(3\\arctan(1/r))$")
    ax.axhline(1, color=C["gray"], linewidth=0.3, ls=":")
    ax.axhline(-1, color=C["gray"], linewidth=0.3, ls=":")
    ax.axhline(0, color=C["gray"], linewidth=0.3)
    ax.set_xlabel("$r$")
    ...
    if data2 is not None:
        ...
        for d, col in [(3, PAL[0]), (5, PAL[1]), (9, PAL[2])]:
            key = f"d{d}_f_found"
            if key in data2:
                ax.plot(r_fine, data2[key], color=col, linewidth=1.0, label=f"$d={d}$")
        ax.set_xlim(0, 10)
        ax.set_ylim(-0.5, 4)
    ax.set_xlabel("$r$")
    ax.set_ylabel("$f(r)$")
