...
(Fig.~\ref{fig:grover-qsp}).  The initial state
...
\begin{figure}[t]
	...
	\begin{minipage}{\columnwidth}
		...
		\centering
		\includegraphics[width=0.65\columnwidth]{figures/grover_bloch.pdf}
	\end{minipage}\\[6pt]
	...
	\caption{%
		...
		\textbf{(a)}~Each application of the signal unitary
		$W(a)$ rotates the state by $\theta = \arccos(a)$ on a
		great circle of the Bloch sphere.  After $k$ applications
		the $\ket{0}$-amplitude is
		$\cos(k\theta) = T_k(a)$, a Chebyshev polynomial of the
		signal~$a$; the dashed line shows the projection
		$T_2(a)$ for $k=2$.
		\textbf{(b)}~The QSP circuit generalizes this structure:
		signal rotations $W(a)$ alternate with tunable phase gates
		$\exp[i\phi_j Z]$.  In Grover's algorithm all phases are
...
at most~$d$ and opposite parity, constrained by
...
$W^k\ket{0} = \cos(k\theta)\ket{0} +
	\sin(k\theta)\ket{1}$.  The Chebyshev polynomials of
