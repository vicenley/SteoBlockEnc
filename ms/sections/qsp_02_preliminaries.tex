...
\begin{theorem}[QSP Theorem~\cite{GSLW2019}]
	...
	\begin{enumerate}
		...
		\item there exists a polynomial $Q$ of degree at
		      most $d-1$ and opposite parity such that
		      $|P(a)|^2 + (1-a^2)|Q(a)|^2 = 1$ for all
		      $a \in \R$,
	\end{enumerate}
...
The polynomial identity in condition~(3) is called the
\emph{complementary polynomial condition}.  Evaluating it
at $a = \pm 1$ forces $|P(\pm 1)| = 1$, since the
$(1-a^2)$ factor vanishes.  This is a necessary condition:
the signal unitary satisfies $W(\pm 1) = \pm I$, so the QSP
product at $a = \pm 1$ collapses to a product of phase gates,
whose diagonal entries have unit modulus.

Theorem~\ref{thm:standard-qsp} is a completeness
result: every polynomial satisfying~(1)--(3) is
achievable, and efficient algorithms exist for
computing the required
phases~\cite{DongMengWhaleyEtAl2021,Haah2019}.
The output state of the QSP circuit is
\begin{equation}
	U_{\bm{\phi}}(a)\ket{0}
	= P(a)\ket{0} + Q(a)\sqrt{1-a^2}\,\ket{1},
	\label{eq:qsp-output-state}
\end{equation}
where $Q$ is the complementary polynomial from
condition~(3).  Both components are bounded by~$1$
in absolute value.
