...
(Section~\ref{sec:cost-analysis}).  Finally, we sketch how the
construction extends to multi-qubit eigenvalue transformations via
stereographic block encoding, illustrating the approach on the
Heisenberg model and identifying the regime---depth-limited devices,
moderate eigenvalues, known eigenbasis---where stereographic encoding
offers a practical advantage (Section~\ref{sec:block-encoding}).
...
Section~\ref{sec:block-encoding} sketches the extension to
multi-qubit eigenvalue transformations via stereographic block
encoding, illustrated on the Heisenberg model.
