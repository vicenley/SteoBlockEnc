...
yet all retain a common limitation: the output remains a bounded
function, $|P| \leq 1$.  No existing QSP variant produces unbounded
output.
...
In this work we ask whether the boundedness constraint can be lifted
without modifying the quantum algorithm itself.  We show that it can,
through a change of the classical encoding and decoding interface.
...
factors from the standard QSP theorem
(Section~\ref{sec:main}).  Finally, we formulate stereographic block
encoding for operators with unbounded spectra, provide explicit
circuit constructions for three Hamiltonian classes---diagonal,
Pauli-$Z$ sum, and Heisenberg---and quantify the measurement
overhead, showing that absolute precision $\epsilon$ requires
$\mathcal{O}(r^6/\epsilon^2)$ shots.  This polynomial sampling cost
replaces the coherent normalization overhead of standard QSP, and we
identify the regime---depth-limited devices, moderate eigenvalues,
known eigenbasis---where stereographic encoding offers a practical
advantage (Section~\ref{sec:block-encoding}).
...
Sections~\ref{sec:main} and \ref{sec:block-encoding} contain the
main results: the stereographic signal operator, Chebyshev state
generation, the basis-change reduction to standard QSP, and the
application to block encoding with unbounded spectra.
