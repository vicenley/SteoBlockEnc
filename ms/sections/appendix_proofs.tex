...
\begin{proof}
	...
	The identity~(A3b) follows from writing
	$\sin(k\phi) = U_{k-1}(\cos\phi)\sin\phi$, which is the
	standard relation between Chebyshev polynomials and
	trigonometric functions (see, e.g.,
	Ref.~\cite{BenentiCasatiStrini2004}).
	...
	normalization: $|T_k(\rtilde)|^2 +
		...
		\sin^2(k\phi) = 1$, confirming unitarity.
\end{proof}
...
\begin{proof}
	...
	\begin{equation}
		...
		= \bra{0}V^{-1}\,U_{\bm{\phi}}^{(\mathrm{std})}(\rtilde)\,V\ket{0}
