...
Stereographic projection provides a fundamentally different
encoding that avoids this restriction.  Projecting from the
north pole of the unit sphere $\Sphere \subset \R^3$ onto the
equatorial plane defines a conformal bijection
\begin{equation}
...
The encoding state~\eqref{eq:encoding} is prepared from
...
$U_3(\vartheta,\varphi,\lambda) \in \SU(2)$ induces the M\"obius
...
$\SU(2) \to \mathrm{PSL}(2,\C)$, the projective special linear
group of $2\times 2$ matrices over $\C$ modulo scalar
multiples---the natural symmetry group of M\"obius
transformations.  In particular, $Z$-rotations act as phase
rotations $R_z(\alpha): z \mapsto e^{i\alpha}z$.
