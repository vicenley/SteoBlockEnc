...
unbounded---it has $\lfloor k/2 \rfloor$ zeros and
$\lfloor(k{-}1)/2\rfloor$ poles on $(0,\infty)$, diverging at
each pole---yet it is produced by a unitary circuit whose
...
\begin{proposition}[Basis Change]
	...
	Let $V = \mathrm{diag}(1,i)$.
	\begin{enumerate}
		\item The stereographic signal operator~\eqref{eq:signal-operator} and the standard signal unitary~\eqref{eq:standard-signal} are related by $S_z = V^{-1}\,W(\rtilde)\,V$ with $\rtilde = r/\sqrt{1+r^2}$.
		\item The phase operator is invariant: $V^{-1}\exp[i\phi\PauliZ]\,V = \exp[i\phi\PauliZ]$.
	\end{enumerate}
	Consequently, writing $W_{\bm{\phi}}^{\mathrm{(stereo)}}(r)$ for the stereographic QSP product~\eqref{eq:stereo-qsp} and $U_{\bm{\phi}}^{\mathrm{(std)}}(\rtilde)$ for the standard QSP product~\eqref{eq:standard-qsp} evaluated at $a = \rtilde$,
	\begin{equation}
...
through the stereographic protocol.  The basis change does
affect the off-diagonal matrix elements: writing the standard
QSP output state as
$P(\rtilde)\ket{0} + iQ(\rtilde)\sqrt{1-\rtilde^2}\,\ket{1}$
(with the standard convention that $Q$ is real for real
targets), the stereographic output state is
$P(\rtilde)\ket{0} + Q(\rtilde)/\sqrt{1+r^2}\,\ket{1}$,
where the factor of~$i$ has been absorbed into the
$\ket{1}$-component by the basis change~$V$.  The decoded
function $f(r) = P(\rtilde)\sqrt{1+r^2}/Q(\rtilde)$ is the
same ratio regardless of which convention is used.
...
and provides the natural language for extending stereographic
QSP to multi-qubit
systems~\cite{KharaziLeytonOrtega2026b}.
...
$\sqrt{2}+1$ are reciprocals of each other, the pole at
$r=0$ maps to the pole at $r \to \infty$ under inversion,
and the pole at $r=1$ is a fixed point of $r \mapsto 1/r$.
...
\begin{table}[t]
	...
	\begin{ruledtabular}
		\begin{tabular}{ccc}
			...
			6   & $2-\sqrt{3},\; 1,\; 2+\sqrt{3}$                 & $0,\; 1/\sqrt{3},\; \sqrt{3}$
		\end{tabular}
	\end{ruledtabular}
\end{table}
...
contributes one factor of $\sqrt{1+r^2}$ to the operator norm
(since $\det(O_r) = 1+r^2$), and squaring gives $(1+r^2)^d$.
...
stereographic map.  For real-valued polynomials $P$ and $Q$,
the QSP output state is
...
\end{equation}
These expressions show that even standard QSP, when decoded
via stereographic projection rather than projective
measurement, produces unbounded rational functions of the
real-line parameter~$u$.  The unboundedness is therefore not
unique to the stereographic protocol; it is a property of the
\emph{decoding interface}, available in principle to any QSP
circuit.  What the stereographic framework provides is a
systematic encoding--decoding pair that makes this mechanism
explicit and connects it to rational Chebyshev approximation
theory.  This provides a dual perspective on our main
construction: stereographic QSP encodes via the stereographic
map before QSP, while
...
$W(a) = a\,\mathbb{I} + i\sqrt{1-a^2}\,\PauliX$ is the
standard signal unitary.  Each phase factor
$\exp[i\phi_k\PauliZ] = \cos\phi_k\,\mathbb{I} +
	i\sin\phi_k\,\PauliZ$ introduces a choice between
$\mathbb{I}$ and $Z$, and the $WZW = Z$ rule collapses
...
\subsection{Generalized QSP in the Stereographic Picture}
\label{subsec:generalized-qsp}

The unnormalized formulation
(Theorem~\ref{thm:unnormalized}) restricts the controls to
$\PauliZ$-rotations, which enforces the same parity constraint
as in standard QSP: the polynomials $\tilde{P}$ and
$\tilde{Q}$ have definite parity, and even- and odd-degree
terms cannot be mixed freely.  In the spirit of generalized
QSP~\cite{MotlaghWiebe2023}, this constraint can be lifted by
replacing the $\PauliZ$-rotation controls with arbitrary
$\SU(2)$ matrices.  The stereographic picture makes this
extension particularly transparent, because the signal and the
controls are parameterized by the same geometric object---a
point on $\C\mathbb{P}^1$.  The following theorem, whose proof
by induction on the polynomial degree is given in
Appendix~\ref{app:proofs}, characterizes the achievable
function class in full generality.

\begin{theorem}[Generalized QSP in Stereo]
	\label{thm:gqsp-stereo}
	Let $z \in \C\mathbb{P}^1$ and define the unnormalized signal
	operator
	\begin{equation}
		O_z = \begin{pmatrix} z & -1 \\ 1 & z^* \end{pmatrix},
		\quad \det O_z = 1 + |z|^2,
		\label{eq:gqsp-signal}
	\end{equation}
	together with $\SU(2)$ control unitaries
	$G_j = \bigl(\begin{smallmatrix}
	a_j & -b_j^* \\ b_j & a_j^* \end{smallmatrix}\bigr)$,
	$j = 0, \ldots, d$.  Then there exist
	$G_0, \ldots, G_d \in \SU(2)$ such that
	\begin{equation}
		G_d \prod_{j=1}^{d} O_z\, G_{j-1}
		= \begin{pmatrix} P(z,z^*) & -Q^*(z,z^*) \\
		Q(z,z^*) & P^*(z,z^*) \end{pmatrix}
		\label{eq:gqsp-product}
	\end{equation}
	if and only if the polynomials $P, Q \in \C[z,z^*]$ satisfy:
	\begin{enumerate}
		\item For all monomials $z^j(z^*)^k$ appearing in $P$ or
		      $Q$, $j + k \leq d$.
		\item $|P(z,z^*)|^2 + |Q(z,z^*)|^2 = (1+|z|^2)^d$ for
		      all $z \in \C$.
	\end{enumerate}
\end{theorem}

The theorem is a genuine extension of both the real
stereographic QSP of Theorem~\ref{thm:unnormalized} and the
generalized QSP of Ref.~\cite{MotlaghWiebe2023}: it permits
polynomials over any $z \in \C\mathbb{P}^1$, not just
$x \in [-1,1]$ or $e^{i\theta} \in \mathbb{T}$, and the
parity constraint is absent.  The normalization condition
$|P|^2 + |Q|^2 = (1+|z|^2)^d$ reduces to the standard QSP
normalization $|P|^2 + |Q|^2 = 1$ on the unit circle
$|z| = 1$ (after rescaling by $2^{-d/2}$), confirming
compatibility with the standard framework.

\textit{Connection to the nonlinear Fourier transform.}
The backward direction of
Theorem~\ref{thm:gqsp-stereo}---given a polynomial pair
$(P,Q)$ satisfying the two conditions, extract the $\SU(2)$
controls $G_j$---is the \emph{layer stripping algorithm}, the
standard method for inverting the $\SU(2)$ nonlinear Fourier
transform (NLFT)~\cite{AlexisBeckerEtAl2026}.  The connection
is direct: the NLFT transfer matrix for a sequence
$(\gamma_0, \ldots, \gamma_{d-1}) \in \C^d$ is the ordered
product
$\hat{\gamma}(z) = \prod_{k=0}^{d-1} (1+|\gamma_k|^2)^{-1/2}
\bigl(\begin{smallmatrix} 1 & \gamma_k z^k \\
-\gamma_k^* z^{-k} & 1 \end{smallmatrix}\bigr)$
for $z \in \mathbb{T}$.  The unnormalized factor
$\bigl(\begin{smallmatrix} 1 & \gamma \\
-\gamma^* & 1 \end{smallmatrix}\bigr)$ is related to the
stereographic signal operator $O_\gamma$ by a fixed $\SU(2)$
element:
\begin{equation}
	\begin{pmatrix} 1 & \gamma \\ -\gamma^* & 1
	\end{pmatrix}
	= O_\gamma \cdot (i\PauliY),
	\label{eq:nlft-identity}
\end{equation}
so the NLFT transfer matrix and the stereographic signal
operator are instances of the same
$\C\mathbb{P}^1$-parameterized family, differing only by
right-multiplication by a fixed $\SU(2)$ element.  When these
matrices are interleaved with varying $\SU(2)$ controls, the
fixed factor $i\PauliY$ can be absorbed into the controls,
making the two product structures equivalent.  The Schur
parameters $\gamma_k$ of the NLFT correspond to the
stereographic controls $G_k$, and the $\C\mathbb{P}^1$
coordinate provides a geometric interpretation: the
Fubini--Study norm $\sqrt{1+|\gamma|^2}$ is the natural
normalization on $\C\mathbb{P}^1$, explaining why this factor
appears in both the NLFT and the stereographic framework.  We
refer the reader to Ref.~\cite{AlexisBeckerEtAl2026} for a
comprehensive treatment of the $\SU(2)$ NLFT and its
