...
\begin{equation}
	\frac{\langle\PauliX\rangle_a + i\langle\PauliY\rangle_a}
	{1 - \langle\PauliZ\rangle_a}
	= \frac{\sum_j |c_j|^2\,P_j\bar{Q}_j}
	{\sum_j |c_j|^2\,|Q_j|^2},
...
An adaptive measurement strategy can improve the scaling
for eigenvalues in a known range.  If $\lambda \in [a, b]$
with $0 < a \leq b$, the Bloch vector lies in a known
region of the sphere, and the decoding formula
$f = (\langle X \rangle + i\langle Y \rangle)
	/ (1 - \langle Z \rangle)$ can be stabilized by allocating
measurement shots to the three Pauli observables in
proportion to their signal-to-noise ratio.  Specifically,
the variance of $\langle Z \rangle$ dominates when $\langle
	Z \rangle \to 1$ (small $\lambda$), so directing more shots
to the $Z$ measurement reduces the error propagation
through the denominator.  A simple two-stage strategy---first
estimate $\langle Z \rangle$ to determine the Bloch
hemisphere, then allocate shots proportionally---can reduce
the effective exponent from $r^6$ to $r^4$ in favorable
cases.  We leave a full analysis of adaptive protocols to
future work.
