...
Several recent developments extend or circumvent the standard
QSP/QSVT framework.  Generalized QSP~\cite{MotlaghWiebe2023}
lifts the parity and realness constraints but retains bounded
polynomial output.  Hamiltonian block
encoding~\cite{KangSu2025} achieves matrix arithmetics with at
most two ancilla qubits, without explicit normalization, yet
still produces bounded transformations.  Randomized
QSVT~\cite{WangZhangHazraEtAl2025} avoids block encoding
entirely through importance sampling, and
Ref.~\cite{MahasingheEtAl2025} uses GQSP with symmetric
combinations of unitary conjugates.  Practical block encoding
circuits for matrix polynomials have been developed using
FOQCS-LCU~\cite{NibbiEtAl2026}, and for bosonic
systems using QSVT itself~\cite{KaneEtAl2025}.  Meanwhile,
...
QSVT circuit depth~\cite{Suresh2026}, and single-ancilla
ground-state preparation without block encoding has been
demonstrated using power-cosine QSP filters~\cite{Jo2026}.
